...
import (
	...
	http1 "net/http"
	"net/url"
	endpoint1 "rocket-server/hello-server/hello-gokit/hello/pkg/endpoint"
	http2 "rocket-server/hello-server/hello-gokit/hello/pkg/http"
...
func New(instance string, options map[string][]http.ClientOption) (service.HelloService, error) {
	...
	{
		sayHelloEndpoint = http.NewClient("GET", copyURL(u, "/say-hello"), encodeHTTPGenericRequest, decodeSayHelloResponse, options["SayHello"]...).Endpoint()
	}

	return endpoint1.Endpoints{SayHelloEndpoint: sayHelloEndpoint}, nil
}
...
// decodeSayHelloResponse is a transport/http.DecodeResponseFunc that decodes
...
func decodeSayHelloResponse(_ context.Context, r *http1.Response) (interface{}, error) {
	if r.StatusCode != http1.StatusOK {
		return nil, http2.ErrorDecoder(r)
	}
	var resp endpoint1.SayHelloResponse
